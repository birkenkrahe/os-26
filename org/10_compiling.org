...
| Stage        | GCC flag | Output    | Notes                                |
|--------------+----------+-----------+--------------------------------------|
| Preprocessor | -E       | .i        | Stops after preprocessing (includes) |
| Assembly     | -S       | .s        | Generates human-readable assembly    |
| Machine code | -c       | .o        | Object file (binary, not yet linked) |
| Linking      | (none)   | a.out/set | Performed unless stopped earlier     |
| Executable   | -o       | custom    | Names the final executable           |

Examples: Write a simple hello world program in C:
...
- Preprocessing: produces =hello.i=
  + Expands ~#include~, macros
  + Output is verbose C code
  #+begin_src bash :results output :exports both
    gcc -E hello.c > hello.i
    ls -l hello*
    file hello.i
    grep printf hello.i # expanded headers
  #+end_src

  #+RESULTS:
  #+begin_example
  -rwxrwxr-x 1 aletheia aletheia 15960 Apr 20 11:39 hello
  -rw-rw-r-- 1 aletheia aletheia    65 Apr 20 11:39 hello.c
  -rw-rw-r-- 1 aletheia aletheia 17950 Apr 20 11:39 hello.i
  -rwxrwxr-x 1 aletheia aletheia  1504 Apr 20 11:38 hello.o
  -rw-rw-r-- 1 aletheia aletheia   685 Apr 20 11:38 hello.s
...
- Assembly:
  ...
  #+RESULTS:
  : -rw-rw-r-- 1 aletheia aletheia    65 Apr 20 11:38 hello.c
  : -rw-rw-r-- 1 aletheia aletheia 17950 Apr 20 11:38 hello.i
  : -rwxrwxr-x 1 aletheia aletheia  1504 Apr 20 11:38 hello.o
  : -rw-rw-r-- 1 aletheia aletheia   685 Apr 20 11:38 hello.s
  : hello.s: assembler source, ASCII text

- Machine code: Assemble to object code =hello.o=
  + Binary, non-executable.
  ...
  : -rw-rw-r-- 1 aletheia aletheia    65 Apr 20 11:38 hello.c
  : -rw-rw-r-- 1 aletheia aletheia 17950 Apr 20 11:38 hello.i
  : -rw-rw-r-- 1 aletheia aletheia  1504 Apr 20 11:38 hello.o
  : -rw-rw-r-- 1 aletheia aletheia   685 Apr 20 11:38 hello.s
  : hello.o: ELF 64-bit LSB relocatable, x86-64, version 1 (SYSV), not stripped

- Linking (usually done by ~ld~ with ~gcc~):
  #+begin_src bash :results output :exports both
    gcc -v hello.c -o hello 2>&1
  #+end_src

* Why are interpreted languages so popular (but compiled languages persist)?
...
- We will compiler a program from the GNU project called ~diction~, a
  style checking tool for English text.

- Create a directory for the source code named ~src~ and then download
  the source code into it using ~ftp~.

  #+begin_example bash
    mkdir -p src/project # (creates missing parent directories)
    cd src/project
    wget https://ftp.gnu.org/gnu/diction/diction-1.11.tar.gz
  #+end_example

- Alternatively, use FTP: You can see the content of the GNU FTP
  server here in a browser: ftp.gnu.org

  Go to https://ftp.gnu.org/pub/gnu/diction/

  Check if you have ~ftp~: =which ftp=. If you don't, install it:
  #+begin_example sh
  sudo apt install ftp
  #+end_example

- Login with =ftp ftp.gnu.org=, username =anonymous= and no password.

- Check out what you can do here with ~help~:
  #+attr_html: :width 400px :float nil:
  [[../img/gnu_ftp.png]]

- Change to ~gnu/diction~ with ~cd~ and ~get~ the latest version of the ~tar~
  archive file. ~ls~ and ~cd~ will work here.
...
- You can also look at the tarball without Emacs:
  + =t= list
  + =z= tells ~tar~ the archive is compressed with ~gzip~ (=tar.gz= or =.tgz=)
  + =v= verbose - detailed information for each file
  + =f= file - specifies the archive filename follows
  #+begin_src bash
    cd src
...
- Unpack the archive with the ~tar~ command:
  + =x= extract file (instead of =t= for listing)
  #+begin_src bash
    cd src
    tar xzvf diction-1.11.tar.gz
    ls -l
  #+end_src
...
- The ~.c~ files contain the two C programs supplied by the package,
  /style/ and /diction/.
...
- E.g. at the top of the ~diction.c~ source file: =nano -v diction.c=
...
- Run configure - prefix the program name with the current directory
  locator (period): run in a shell - the command produces a lot of
  messages.
...
- All the targets from the ~Makefile~ are now present in our
  directory, including the main programs ~diction~ and ~style~. Run ~ls~ to
  confirm this.
...
- The ~make~ program only builds what needs building and checks all
  dependencies. To show this, get rid of some intermediate programs
  and run ~make~ again:
...
- You see that ~make~ rebuilds and relinks the main programs ~diction~ and
  ~style~ because they depend on the missing module.

- ~make~ also keeps targets up to date and ensures that all code is
  built using the most recent source code.

- Use the ~touch~ program to "update" one of the source code files - as
  if a programmer had changed ~getopt.c~.
...
- ~getopt.c~ is now more recent than the built file, and ~make~ will
  discover and restore the target to being newer than the dependency.
...
- To clean the directory from all intermediate files, run =make
  clean=. Only the installation files remain.

* Installing the program

The special ~make~ target ~install~ will install the final product in the
operating system ready for use. Usually, this is ~/usr/local/bin~, which
is not writable for regular users, so we must use ~sudo~.
...
#+begin_src bash :results output :exports both
  cd ~
  diction --version  # version you installed
  which diction # system-wide installation b/c of sudo
#+end_src

Create a test file:
#+begin_src bash :results output :exports both
  echo "This is a very very simple test sentence which is actually quite \
  redundant." > test.txt
  cat test.txt
#+end_src

#+RESULTS:
: This is a very very simple test sentence which is actually quite redundant.

Running ~diction~ on =test.txt= will flag words that are redundant, or
awkward phrases and make inline suggestions.

This works best in connection with ~deroff~ (strips formatting):
#+begin_src bash :results output :exports both
deroff test.txt | diction
#+end_src

~deroff~ will not be available on Cloud shell, and like ~diction~, if you
want it you'll have to build it from scratch.

Try verbose/style options:
#+begin_src bash :results output :exports both
diction -bs test.txt
#+end_src
...
#+begin_src bash :results output :exports both
  ...
  man ls | col -b | diction  # col -b filters lines, removes ctrl chars
#+end_src

Install ~fortune~ then run ~diction~ on it:
#+begin_src bash :results output :exports both
  sudo apt install fortune
  /usr/games/fortune
  /usr/games/fortune | diction [-bs]
#+end_src

To add =/usr/games/= to the =PATH=:
#+begin_src bash :results output :exports both
  export PATH=$PATH:/usr/games
  fortune | tee fortune.txt | diction
#+end_src
