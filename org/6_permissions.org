...
- Check ~/home~ where your ~$HOME~ is. What are the permissions, and what
  ...
  #+begin_quote
  Answer: you and your group can enter and read, only you can write to
  ~$HOME~
  #+end_quote
...
- Most languages have conversion functions for different bases,
  ...
  #+begin_example python
    ...
    oct(50)  # octal to decimal (40)
    oct(10)  # octal to binary  (1000)
  #+end_example

- You can also do this on the command line with ~bc~:
  #+begin_src bash :results output :exports both
    echo "obase=8; ibase=10; 8" | bc # 8^1 + 0 * 8^0
    echo "obase=2; ibase=10; 8" | bc # 2^3 + 0 * (2^2+2^1+2^0)
    echo "obase=10; ibase=8; 50"| bc # 5 * 8^1 + 0 * 8^0
    echo "obase=2; ibase=8; 7" | bc # 4 + 2 + 1
    echo "obase=2; ibase=8; 6" | bc # 4 + 2 + 0        
    echo "obase=2; ibase=8; 5" | bc # 4 + 0 + 1   
  #+end_src
...
- So if you can set permissions with symbols, why would you bother
  with octal?
  #+begin_quote
  Octal matches the 3-bit structure of permissions exactly, aligns
  with internal binary representations, and is very compact. It's best
  suited for bulk assignments (rather than individual file changes).
  #+end_quote

- You can also set permissions in the GUI but that's not stable, not
  fast, not consistent, and not suited to bulk permission changes.
  #+attr_html: :width 400px :float nil:
  [[../img/gui_permissions.png]]
...
- When a file is created, the =umask= command expresses a /mask/ of bits
  to be *removed* from from the default mode attributes of a file.

- What is the default file mode for files and directories?
  #+begin_quote
  The default file mode is 666 (rw-rw-rw). The default directory mode
  is 777 (rwxrwxrwx).
  #+end_quote
...
- How can you find this out?
  #+begin_src bash :results output :exports both
    rm -rf empty*
    touch empty
    ls -l empty  # -rw-rw-r-- does not seem to be right: umask is applied
    mkdir empty2
    ls -ld empty*
  #+end_src

  #+RESULTS:
  : -rw-rw-r-- 1 aletheia aletheia 0 Mar 30 08:00 empty
  : -rw-rw-r-- 1 aletheia aletheia    0 Mar 30 08:00 empty
  : drwxrwxr-x 2 aletheia aletheia 4096 Mar 30 08:00 empty2
...
- So what did ~umask~ do?
  #+begin_quote
  umask 002 removes the write permission from the 'others' group. It
  is applied to the default file mode: 666-002=664 or rw-rw-r.
  #+end_quote  
...
- Listing with ~ls -l~ shows the special permissions. Here is an example
  ...
  : -rwSrw-r-- 1 aletheia aletheia 0 Mar 30 08:17 empty
...
- Here is an example of assigning =setgid= to a directory:
  ...
  : drwxrwsr-x 2 aletheia aletheia 4096 Mar 30 08:17 Empty
...
- Here is an example of a directory with the sticky bit set:
  ...
  : mkdir: created directory 'Empty'
  : drwxrwxr-t 2 aletheia aletheia 4096 Mar 30 08:18 Empty
...
* IN PROGRESS Change file owner and group (=chown=) 
