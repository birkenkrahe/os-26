...
- [X] Computer system components
- [ ] The OS kernel and *interrupts* (special system call)
...
- An operating system is software that manages a computer's
  hardware.
- Before 1960s: every system action had to be programmed
  explicitly.
- Early 1960s: allowed programmers to focus on application
  development.
...
What issues could any of these actors have? What could go wrong?
#+begin_notes
...
* What's the kernel? Why is it called that?
#+begin_quote
...
#+begin_example python :results none :session *Python* :tangle kernel.py
...
Recall the difference between device adapters vs. controllers:
...
This will also work on Google Cloud Shell (do it) showing that this
system is a proper Linux VM with its own kernel.
