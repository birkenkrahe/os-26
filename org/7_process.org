...
#+SUBTITLE:processes practice for CSC420 Operating Systems Spring 2026 Lyon College
...
* Controlling processes (using ~nano~)

** Interrupting a process

+ As a guinea pig program, we use ~nano~.

+ [ ] Open a terminal and enter ~nano~ at the prompt. A new nano editor appears. Notice that the terminal prompt does not return.

+ [ ] Exit nano normally by pressing ~CTRL-X~ (and confirm if prompted). The shell prompt returns.

+ [ ] Enter ~nano~ again in the shell, and interrupt it with ~CTRL-C~.

+ Many programs can be interrupted this way by sending an *interrupt*
  signal to the *kernel*.

+ This may not work with ~nano~. Try ~sleep 10~ and stop it with CTRL-Z

+ Look at ~sleep~ with ~sleep --help~ first
...
+ [ ] Start nano from the shell /in the background/ using:
  ~nano &~
  Observe what happens.

+ The message that appears is part of shell /job control/. It shows
  the job number and PID.

  #+begin_example
  [1] 13899
  #+end_example

+ [ ] Check the process table with ~ps~.

+ [ ] ~grep~ the ~nano~ process from the process table using the
...
+ A process in the background is immune from keyboard input. To
  return it to the foreground, use ~fg~.

+ [ ] On the same shell, return the process to the foreground with:
  ~fg %1~

+ [ ] Exit nano using ~CTRL-X~.

+ =%%= or =%+= refers to the current job (the one most recently started or
  suspended). You can also use =%string= where =string= is a command
  string, e.g. =%sl= if you're looking for ~sleep~.

** Stopping or pausing a process

- Start ~nano~ in the foreground.

- Press ~CTRL-T CTRL-Z~ to stop (pause) the process.
...
- [ ] Resume the process:
  - Bring it to the foreground: ~fg %1~
  - Or continue it in the background: ~bg %1~

- *Why launch programs from the shell?*
  - Not all programs are available in the GUI
  - You can see error messages
  - You can use command-line options

- Notice: If you started with ~nano &~, the process is already in the
  background and ~bg %1~ does nothing. It will only work on *stopped*
  processes. So start ~nano~, stop it with ~CTRL-T CTRL-Z~ and then
  push it into the background with ~bg %1~.

- Example:
  + =sleep 100= starts a job

  + =CTRL-Z= stops it

  + check with =jobs= that it is stopped
...
+ Start nano in the background: ~nano &~

+ Find its PID using ~ps~.

+ Kill the process: ~kill PID~

+ May have to use ~kill -9 PID~ (~SIGKILL~) because ~SIGTERM~ is handled
  with a delay
...
* SOMEDAY Controlling processes (with ~emacs~)
